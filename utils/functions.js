...
export async function findProductsWithOrders() {
  ...
  try {
    ...
    for (const productDoc of productsQuerySnapshot.docs) {
      ...
      const ordersQuerySnapshot = await getDocs(collection(doc(db, 'products', productId), 'orders'));

      if (!ordersQuerySnapshot.empty) {
        // If the product has orders, add it to the list of products with orders
        productsWithOrders.push({
          productId: productId,
          productData: productDoc.data(),
          orders: ordersQuerySnapshot.docs.map(orderDoc => orderDoc.data())
        });
      }
    }
    ...
    console.error("Error retrieving products with orders:", error);
...
export const getSubcollectionDocuments = (subcollectionName, callback) => {
  const collectionGroupRef = doc(db, "products", "");
